...
class TestMobileCountryDirect:

    def test_should_direct_load_mobile_country(self):
        setup = _mobile_country_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-mobile_country", "live" if setup["live"] else "unit")
        if _skip:
...
def _mobile_country_direct_setup(mockres):
