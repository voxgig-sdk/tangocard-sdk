# LowBalanceAlertView entity test

import json
import os
import time

import pytest

from tangocard_sdk.utility.voxgig_struct import voxgig_struct as vs
from tangocard_sdk import TangocardSDK
from tangocard_sdk.core import helpers

_TEST_DIR = os.path.dirname(os.path.abspath(__file__))
from test import runner


class TestLowBalanceAlertViewEntity:

    def test_should_create_instance(self):
        testsdk = TangocardSDK.test(None, None)
        ent = testsdk.LowBalanceAlertView(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _low_balance_alert_view_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "low_balance_alert_view." + _op, "live" if _live else "unit")
            if _skip:
                pytest.skip(_reason or "skipped via sdk-test-control.json")
                return
        # The basic flow consumes synthetic IDs from the fixture. In live mode
        # without an *_ENTID env override, those IDs hit the live API and 4xx.
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TANGOCARD_TEST_LOW_BALANCE_ALERT_VIEW_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        low_balance_alert_view_ref01_ent = client.LowBalanceAlertView(None)
        low_balance_alert_view_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.low_balance_alert_view"), "low_balance_alert_view_ref01"))
        low_balance_alert_view_ref01_data["account_id"] = setup["idmap"]["account01"]
        low_balance_alert_view_ref01_data["account_identifier"] = setup["idmap"]["accountentifier01"]
        low_balance_alert_view_ref01_data["customer_identifier"] = setup["idmap"]["customerentifier01"]

        low_balance_alert_view_ref01_data = helpers.to_map(runner.entity_data(low_balance_alert_view_ref01_ent.create(low_balance_alert_view_ref01_data, None)))
        assert low_balance_alert_view_ref01_data is not None

        # UPDATE
        low_balance_alert_view_ref01_data_up0_up = {
            "account_id": setup["idmap"]["account_id"],
            "customer_identifier": setup["idmap"]["customer_identifier"],
        }

        low_balance_alert_view_ref01_markdef_up0_name = "accountIdentifier"
        low_balance_alert_view_ref01_markdef_up0_value = "Mark01-low_balance_alert_view_ref01_" + str(setup["now"])
        low_balance_alert_view_ref01_data_up0_up[low_balance_alert_view_ref01_markdef_up0_name] = low_balance_alert_view_ref01_markdef_up0_value

        low_balance_alert_view_ref01_resdata_up0 = helpers.to_map(runner.entity_data(low_balance_alert_view_ref01_ent.update(low_balance_alert_view_ref01_data_up0_up, None)))
        assert low_balance_alert_view_ref01_resdata_up0 is not None
        assert low_balance_alert_view_ref01_resdata_up0[low_balance_alert_view_ref01_markdef_up0_name] == low_balance_alert_view_ref01_markdef_up0_value

        # LOAD
        low_balance_alert_view_ref01_match_dt0 = {}
        low_balance_alert_view_ref01_data_dt0_loaded = low_balance_alert_view_ref01_ent.load(low_balance_alert_view_ref01_match_dt0, None)
        assert low_balance_alert_view_ref01_data_dt0_loaded is not None



def _low_balance_alert_view_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/low_balance_alert_view/LowBalanceAlertViewTestData.json")
    with open(entity_data_file, "r") as f:
        entity_data_source = f.read()

    entity_data = json.loads(entity_data_source)

    options = {}
    options["entity"] = entity_data.get("existing")

    client = TangocardSDK.test(options, extra)

    # Generate idmap via transform.
    idmap = vs.transform(
        ["low_balance_alert_view01", "low_balance_alert_view02", "low_balance_alert_view03", "customer01", "customer02", "customer03", "account01", "account02", "account03", "lowbalance01", "lowbalance02", "lowbalance03", "accountentifier01", "customerentifier01"],
        {
            "`$PACK`": ["", {
                "`$KEY`": "`$COPY`",
                "`$VAL`": ["`$FORMAT`", "upper", "`$COPY`"],
            }],
        }
    )

    # Detect ENTID env override before envOverride consumes it. When live
    # mode is on without a real override, the basic test runs against synthetic
    # IDs from the fixture and 4xx's. We surface this so the test can skip.
    _entid_env_raw = os.environ.get(
        "TANGOCARD_TEST_LOW_BALANCE_ALERT_VIEW_ENTID")
    _idmap_overridden = _entid_env_raw is not None and _entid_env_raw.strip().startswith("{")

    env = runner.env_override({
        "TANGOCARD_TEST_LOW_BALANCE_ALERT_VIEW_ENTID": idmap,
        "TANGOCARD_TEST_LIVE": "FALSE",
        "TANGOCARD_TEST_EXPLAIN": "FALSE",
        "TANGOCARD_APIKEY": "",
    })

    idmap_resolved = helpers.to_map(
        env.get("TANGOCARD_TEST_LOW_BALANCE_ALERT_VIEW_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)
    if idmap_resolved.get("account_id") is None:
        idmap_resolved["account_id"] = idmap_resolved.get("account01")
    if idmap_resolved.get("customer_identifier") is None:
        idmap_resolved["customer_identifier"] = idmap_resolved.get("customerentifier01")

    if env.get("TANGOCARD_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            # FIRST, so the generated fields below win: sdk-test-control.json's
            # test.client.options adds to the live client, it does not
            # redirect it.
            runner.live_client_options(),
            {
                "apikey": env.get("TANGOCARD_APIKEY"),
            },
            extra or {},
        ])
        client = TangocardSDK(helpers.to_map(merged_opts))

    _live = env.get("TANGOCARD_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("TANGOCARD_TEST_EXPLAIN") == "TRUE",
        "live": _live,
        "synthetic_only": _live and not _idmap_overridden,
        "now": int(time.time() * 1000),
    }
