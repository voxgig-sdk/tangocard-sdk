...
class TestAsyncOrderDirect:

    def test_should_direct_list_async_order(self):
        setup = _async_order_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-async_order", "live" if setup["live"] else "unit")
        if _skip:
...
def _async_order_direct_setup(mockres):
