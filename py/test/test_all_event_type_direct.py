...
class TestAllEventTypeDirect:

    def test_should_direct_list_all_event_type(self):
        setup = _all_event_type_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-all_event_type", "live" if setup["live"] else "unit")
        if _skip:
...
def _all_event_type_direct_setup(mockres):
