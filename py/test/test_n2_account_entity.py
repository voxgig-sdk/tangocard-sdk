...
class TestN2AccountEntity:

    def test_should_create_instance(self):
        testsdk = TangocardSDK.test(None, None)
        ent = testsdk.N2Account(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _n2_account_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "n2_account." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TANGOCARD_TEST_N2_ACCOUNT_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        n2_account_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.n2_account")))
        n2_account_ref01_data = None
        if len(n2_account_ref01_data_raw) > 0:
            n2_account_ref01_data = helpers.to_map(n2_account_ref01_data_raw[0][1])



def _n2_account_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/n2_account/N2AccountTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "TANGOCARD_TEST_N2_ACCOUNT_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TANGOCARD_TEST_N2_ACCOUNT_ENTID"))
