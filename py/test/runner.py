...
class TangocardTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TangocardTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TangocardTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TangocardTestRunner.getenv("TANGOCARD_TEST_LIVE")
        override = TangocardTestRunner.getenv("TANGOCARD_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TangocardTestRunner.getenv(key)
        ...
        explain = TangocardTestRunner.getenv("TANGOCARD_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TANGOCARD_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TangocardTestRunner._test_control is not None:
            return TangocardTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TangocardTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TangocardTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TangocardTestRunner.load_test_control()
...
def load_env_local():
    TangocardTestRunner.load_env_local()


def env_override(m):
    return TangocardTestRunner.env_override(m)


def entity_data(v):
    return TangocardTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TangocardTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TangocardTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TangocardTestRunner.load_test_control()


def live_client_options():
    return TangocardTestRunner.live_client_options()


def live_delay_ms():
    return TangocardTestRunner.live_delay_ms()
