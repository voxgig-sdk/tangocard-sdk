...
class MobileCountryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return MobileCountryEntity(self._client, opts)
    ...
    def data_get(self) -> MobileCountry:
    ...
    def match_get(self) -> MobileCountry:
    ...
    def load(self, reqmatch=None, ctrl=None) -> MobileCountry:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.MobileCountry().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
