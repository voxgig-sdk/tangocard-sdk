...
class AsyncOrderEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AsyncOrderEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[AsyncOrder]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.AsyncOrder().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: AsyncOrderCreateData, ctrl=None) -> AsyncOrder:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
