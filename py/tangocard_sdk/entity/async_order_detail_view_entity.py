...
class AsyncOrderDetailViewEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AsyncOrderDetailViewEntity(self._client, opts)
    ...
    def data_get(self) -> AsyncOrderDetailView:
    ...
    def match_get(self) -> AsyncOrderDetailView:
    ...
    def load(self, reqmatch=None, ctrl=None) -> AsyncOrderDetailView:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.AsyncOrderDetailView().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: AsyncOrderDetailViewUpdateData, ctrl=None) -> AsyncOrderDetailView:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
