...
class N9DigitalTemplateEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return N9DigitalTemplateEntity(self._client, opts)
    ...
    def data_get(self) -> N9DigitalTemplate:
    ...
    def match_get(self) -> N9DigitalTemplate:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> N9DigitalTemplate:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.N9DigitalTemplate().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
