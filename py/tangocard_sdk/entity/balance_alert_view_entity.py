...
class BalanceAlertViewEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BalanceAlertViewEntity(self._client, opts)
    ...
    def data_get(self) -> BalanceAlertView:
    ...
    def match_get(self) -> BalanceAlertView:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> BalanceAlertView:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.BalanceAlertView().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
