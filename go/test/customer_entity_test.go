...
func TestCustomerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		customerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.customer", setup.data)))
...
func customerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TANGOCARD_TEST_CUSTOMER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TANGOCARD_TEST_CUSTOMER_ENTID"])
