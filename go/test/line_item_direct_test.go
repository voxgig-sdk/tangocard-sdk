...
func TestLineItemDirect(t *testing.T) {
	t.Run("direct-list-line_item", func(t *testing.T) {
		setup := line_itemDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-line_item", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-line_item", func(t *testing.T) {
		setup := line_itemDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-line_item", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type line_itemDirectSetupResult struct {
...
func line_itemDirectSetup(mockres any) *line_itemDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"TANGOCARD_TEST_LINE_ITEM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["TANGOCARD_TEST_LINE_ITEM_ENTID"]; ok {
		...
		return &line_itemDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &line_itemDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
