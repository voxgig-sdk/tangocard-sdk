...
import (
	...
	omni "github.com/voxgig-sdk/tangocard-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TangocardError); is {
				out := map[string]any{
					"name":    "TangocardError",
...
func omniCtx(arg any, client *sdk.TangocardSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TangocardSDK); is {
			client = live
