...
func TestWebhookEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TANGOCARD_TEST_WEBHOOK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		webhookRef01Data["webhook_id"] = setup.idmap["webhook01"]
...
func webhookBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TANGOCARD_TEST_WEBHOOK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TANGOCARD_TEST_WEBHOOK_ENTID"])
