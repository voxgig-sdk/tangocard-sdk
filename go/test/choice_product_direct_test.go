...
func TestChoiceProductDirect(t *testing.T) {
	t.Run("direct-list-choice_product", func(t *testing.T) {
		setup := choice_productDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-choice_product", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-choice_product", func(t *testing.T) {
		setup := choice_productDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-choice_product", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type choice_productDirectSetupResult struct {
...
func choice_productDirectSetup(mockres any) *choice_productDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"TANGOCARD_TEST_CHOICE_PRODUCT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["TANGOCARD_TEST_CHOICE_PRODUCT_ENTID"]; ok {
		...
		return &choice_productDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &choice_productDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
