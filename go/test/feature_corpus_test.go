...
//
// NewTangocardSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.TangocardSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewTangocardSDK(opts)
}
...
func fcCandidates(client *sdk.TangocardSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.TangocardSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.TangocardSDK, name string) any {
