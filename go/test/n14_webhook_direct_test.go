...
func TestN14WebhookDirect(t *testing.T) {
	t.Run("direct-list-n14_webhook", func(t *testing.T) {
		setup := n14_webhookDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-n14_webhook", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-n14_webhook", func(t *testing.T) {
		setup := n14_webhookDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-n14_webhook", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type n14_webhookDirectSetupResult struct {
...
func n14_webhookDirectSetup(mockres any) *n14_webhookDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"TANGOCARD_TEST_N14_WEBHOOK_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["TANGOCARD_TEST_N14_WEBHOOK_ENTID"]; ok {
		...
		return &n14_webhookDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &n14_webhookDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
