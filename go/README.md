...
It exposes the API as capitalised, semantic **Entities** — e.g. `client.Catalog(nil)` — each with the same small set of operations (`List`, `Create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
func main() {
    ...
    // List catalog records — the value is the array of records itself.
    catalogs, err := client.Catalog(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range catalogs.([]any) {
        fmt.Println(item)
    }
}
```
...
slice):
    ...
    catalog, err := client.Catalog(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
