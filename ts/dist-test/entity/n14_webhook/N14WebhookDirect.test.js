...
(0, node_test_1.describe)('N14WebhookDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-n14_webhook', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-n14_webhook', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["webhook01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.webhook_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.webhook_id = candidateId;
            params.webhook_id = setup.idmap['webhook01'];
        }
        else {
            params.webhook_id = 'direct01';
        }
    ...
    (0, node_test_1.test)('direct-list-n14_webhook', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-n14_webhook', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['TANGOCARD_TEST_N14_WEBHOOK_ENTID'];
...
//# sourceMappingURL=N14WebhookDirect.test.js.map