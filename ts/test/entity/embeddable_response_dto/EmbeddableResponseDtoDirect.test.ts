...
describe('EmbeddableResponseDtoDirect', async () => {
  ...
  test('direct-load-embeddable_response_dto', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-embeddable_response_dto', setup.live)) return
    if (skipIfMissingIds(t, setup, ["reference_line_item_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_EMBEDDABLE_RESPONSE_DTO_ENTID']
