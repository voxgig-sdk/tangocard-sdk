...
describe('CredentialTypeViewEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'credential_type_view.' + op, live)) return
    }
    ...
    let credential_type_view_ref01_data = Object.values(setup.data.existing.credential_type_view)[0] as any

    // LIST
    const credential_type_view_ref01_ent = client.CredentialTypeView()
    const credential_type_view_ref01_match: any = {}

    const credential_type_view_ref01_list = (await credential_type_view_ref01_ent.list(credential_type_view_ref01_match)).map((e: any) => e.data())
