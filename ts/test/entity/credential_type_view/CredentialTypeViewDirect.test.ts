...
describe('CredentialTypeViewDirect', async () => {
  ...
  test('direct-list-credential_type_view', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-credential_type_view', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_CREDENTIAL_TYPE_VIEW_ENTID']
