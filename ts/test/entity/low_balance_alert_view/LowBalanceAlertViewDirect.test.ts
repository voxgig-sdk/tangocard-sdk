...
describe('LowBalanceAlertViewDirect', async () => {
  ...
  test('direct-load-low_balance_alert_view', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-low_balance_alert_view', setup.live)) return
    if (skipIfMissingIds(t, setup, ["account_id01","balance_alert_id01","customer_identifier01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_LOW_BALANCE_ALERT_VIEW_ENTID']
