...
describe('AsyncOrderLineItemsViewDirect', async () => {
  ...
  test('direct-list-async_order_line_items_view', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-async_order_line_items_view', setup.live)) return
    if (skipIfMissingIds(t, setup, ["account01","customer01","external_ref01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_ASYNC_ORDER_LINE_ITEMS_VIEW_ENTID']
