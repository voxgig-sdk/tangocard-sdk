...
describe('AsyncReasonCodesViewDirect', async () => {
  ...
  test('direct-load-async_reason_codes_view', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-async_reason_codes_view', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_ASYNC_REASON_CODES_VIEW_ENTID']
