...
describe('MobileCountryDirect', async () => {
  ...
  test('direct-load-mobile_country', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-mobile_country', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_MOBILE_COUNTRY_ENTID']
