...
describe('AsyncOrderDetailViewDirect', async () => {
  ...
  test('direct-load-async_order_detail_view', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-async_order_detail_view', setup.live)) return
    if (skipIfMissingIds(t, setup, ["account_identifier01","customer_identifier01","external_ref_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_ASYNC_ORDER_DETAIL_VIEW_ENTID']
