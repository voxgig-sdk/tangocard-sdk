...
describe('AllEventTypeDirect', async () => {
  ...
  test('direct-list-all_event_type', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-all_event_type', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_ALL_EVENT_TYPE_ENTID']
