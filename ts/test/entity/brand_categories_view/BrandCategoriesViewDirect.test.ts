...
describe('BrandCategoriesViewDirect', async () => {
  ...
  test('direct-list-brand_categories_view', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-brand_categories_view', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_BRAND_CATEGORIES_VIEW_ENTID']
