...
describe('CountryViewSummaryDirect', async () => {
  ...
  test('direct-load-country_view_summary', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-country_view_summary', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_COUNTRY_VIEW_SUMMARY_ENTID']
