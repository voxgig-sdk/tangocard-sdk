...
describe('N1CustomerDirect', async () => {
  ...
  test('direct-load-n1_customer', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-n1_customer', setup.live)) return
    if (skipIfMissingIds(t, setup, ["customer_identifier01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_N1_CUSTOMER_ENTID']
