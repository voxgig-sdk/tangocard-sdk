...
describe('RewardReasonsMapDirect', async () => {
  ...
  test('direct-load-reward_reasons_map', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-reward_reasons_map', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_REWARD_REASONS_MAP_ENTID']
