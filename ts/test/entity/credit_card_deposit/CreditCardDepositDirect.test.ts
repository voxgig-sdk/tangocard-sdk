...
describe('CreditCardDepositDirect', async () => {
  ...
  test('direct-load-credit_card_deposit', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-credit_card_deposit', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_CREDIT_CARD_DEPOSIT_ENTID']
