...
describe('EmailTemplateViewVerboseDirect', async () => {
  ...
  test('direct-load-email_template_view_verbose', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-email_template_view_verbose', setup.live)) return
    if (skipIfMissingIds(t, setup, ["etid01"])) return
  ...
  test('direct-list-email_template_view_verbose', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-email_template_view_verbose', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TANGOCARD_TEST_EMAIL_TEMPLATE_VIEW_VERBOSE_ENTID']
