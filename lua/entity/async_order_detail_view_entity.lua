...
local AsyncOrderDetailViewEntity = {}
AsyncOrderDetailViewEntity.__index = AsyncOrderDetailViewEntity


function AsyncOrderDetailViewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AsyncOrderDetailViewEntity)
  self._name = "async_order_detail_view"
  self._client = client
...
function AsyncOrderDetailViewEntity:get_name()
  return self._name
end


function AsyncOrderDetailViewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AsyncOrderDetailViewEntity.new(self._client, opts)
end
...
function AsyncOrderDetailViewEntity:mark_deleted()
  self._deleted = true
end


function AsyncOrderDetailViewEntity:deleted()
  return true == self._deleted
end


function AsyncOrderDetailViewEntity:data_set(args)
  if args ~= nil then
...
function AsyncOrderDetailViewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AsyncOrderDetailViewEntity:match_set(args)
  if args ~= nil then
...
function AsyncOrderDetailViewEntity:match_get()
...
function AsyncOrderDetailViewEntity:stream(action, args, callopts)
...
---@param reqmatch AsyncOrderDetailViewLoadMatch
---@param ctrl? table
---@return AsyncOrderDetailView
---@return string? err
function AsyncOrderDetailViewEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AsyncOrderDetailViewUpdateData
---@param ctrl? table
---@return AsyncOrderDetailView
---@return string? err
function AsyncOrderDetailViewEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function AsyncOrderDetailViewEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AsyncOrderDetailViewEntity
