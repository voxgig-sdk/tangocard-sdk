...
local PrepaidCardInfoEntity = {}
PrepaidCardInfoEntity.__index = PrepaidCardInfoEntity


function PrepaidCardInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PrepaidCardInfoEntity)
  self._name = "prepaid_card_info"
  self._client = client
...
function PrepaidCardInfoEntity:get_name()
  return self._name
end


function PrepaidCardInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PrepaidCardInfoEntity.new(self._client, opts)
end
...
function PrepaidCardInfoEntity:mark_deleted()
  self._deleted = true
end


function PrepaidCardInfoEntity:deleted()
  return true == self._deleted
end


function PrepaidCardInfoEntity:data_set(args)
  if args ~= nil then
...
function PrepaidCardInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PrepaidCardInfoEntity:match_set(args)
  if args ~= nil then
...
function PrepaidCardInfoEntity:match_get()
...
function PrepaidCardInfoEntity:stream(action, args, callopts)
...
---@param reqmatch PrepaidCardInfoLoadMatch
---@param ctrl? table
---@return PrepaidCardInfo
---@return string? err
function PrepaidCardInfoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PrepaidCardInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
