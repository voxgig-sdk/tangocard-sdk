...
local LineItemEntity = {}
LineItemEntity.__index = LineItemEntity


function LineItemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LineItemEntity)
  self._name = "line_item"
  self._client = client
...
function LineItemEntity:get_name()
  return self._name
end


function LineItemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LineItemEntity.new(self._client, opts)
end
...
function LineItemEntity:mark_deleted()
  self._deleted = true
end


function LineItemEntity:deleted()
  return true == self._deleted
end


function LineItemEntity:data_set(args)
  if args ~= nil then
...
function LineItemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LineItemEntity:match_set(args)
  if args ~= nil then
...
function LineItemEntity:match_get()
...
function LineItemEntity:stream(action, args, callopts)
...
---@param reqmatch LineItemLoadMatch
---@param ctrl? table
---@return LineItem
---@return string? err
function LineItemEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch LineItemListMatch
---@param ctrl? table
---@return LineItem[]
---@return string? err
function LineItemEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata LineItemCreateData
---@param ctrl? table
---@return LineItem
---@return string? err
function LineItemEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function LineItemEntity:_run_op(ctx, post_done)
  local utility = self._utility
