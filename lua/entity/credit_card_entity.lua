...
local CreditCardEntity = {}
CreditCardEntity.__index = CreditCardEntity


function CreditCardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CreditCardEntity)
  self._name = "credit_card"
  self._client = client
...
function CreditCardEntity:get_name()
  return self._name
end


function CreditCardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CreditCardEntity.new(self._client, opts)
end
...
function CreditCardEntity:mark_deleted()
  self._deleted = true
end


function CreditCardEntity:deleted()
  return true == self._deleted
end


function CreditCardEntity:data_set(args)
  if args ~= nil then
...
function CreditCardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CreditCardEntity:match_set(args)
  if args ~= nil then
...
function CreditCardEntity:match_get()
...
function CreditCardEntity:stream(action, args, callopts)
...
---@param reqmatch CreditCardLoadMatch
---@param ctrl? table
---@return CreditCard
---@return string? err
function CreditCardEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CreditCardCreateData
---@param ctrl? table
---@return CreditCard
---@return string? err
function CreditCardEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CreditCardEntity:_run_op(ctx, post_done)
  local utility = self._utility
