...
local MobileCountryEntity = {}
MobileCountryEntity.__index = MobileCountryEntity


function MobileCountryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MobileCountryEntity)
  self._name = "mobile_country"
  self._client = client
...
function MobileCountryEntity:get_name()
  return self._name
end


function MobileCountryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MobileCountryEntity.new(self._client, opts)
end
...
function MobileCountryEntity:mark_deleted()
  self._deleted = true
end


function MobileCountryEntity:deleted()
  return true == self._deleted
end


function MobileCountryEntity:data_set(args)
  if args ~= nil then
...
function MobileCountryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MobileCountryEntity:match_set(args)
  if args ~= nil then
...
function MobileCountryEntity:match_get()
...
function MobileCountryEntity:stream(action, args, callopts)
...
---@param reqmatch MobileCountryLoadMatch
---@param ctrl? table
---@return MobileCountry
---@return string? err
function MobileCountryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function MobileCountryEntity:_run_op(ctx, post_done)
  local utility = self._utility
