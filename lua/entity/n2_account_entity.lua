-- Tangocard SDK N2Account entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local N2AccountEntity = {}
N2AccountEntity.__index = N2AccountEntity


function N2AccountEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, N2AccountEntity)
  self._name = "n2_account"
  self._client = client
...
function N2AccountEntity:get_name()
  return self._name
end


function N2AccountEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return N2AccountEntity.new(self._client, opts)
end
...
function N2AccountEntity:mark_deleted()
  self._deleted = true
end


function N2AccountEntity:deleted()
  return true == self._deleted
end


function N2AccountEntity:data_set(args)
  if args ~= nil then
...
function N2AccountEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function N2AccountEntity:match_set(args)
  if args ~= nil then
...
function N2AccountEntity:match_get()
...
function N2AccountEntity:stream(action, args, callopts)
...
function N2AccountEntity:_run_op(ctx, post_done)
  local utility = self._utility
