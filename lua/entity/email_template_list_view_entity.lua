-- Tangocard SDK EmailTemplateListView entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EmailTemplateListViewEntity = {}
EmailTemplateListViewEntity.__index = EmailTemplateListViewEntity


function EmailTemplateListViewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmailTemplateListViewEntity)
  self._name = "email_template_list_view"
  self._client = client
...
function EmailTemplateListViewEntity:get_name()
  return self._name
end


function EmailTemplateListViewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmailTemplateListViewEntity.new(self._client, opts)
end
...
function EmailTemplateListViewEntity:mark_deleted()
  self._deleted = true
end


function EmailTemplateListViewEntity:deleted()
  return true == self._deleted
end


function EmailTemplateListViewEntity:data_set(args)
  if args ~= nil then
...
function EmailTemplateListViewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmailTemplateListViewEntity:match_set(args)
  if args ~= nil then
...
function EmailTemplateListViewEntity:match_get()
...
function EmailTemplateListViewEntity:stream(action, args, callopts)
...
function EmailTemplateListViewEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return EmailTemplateListViewEntity
