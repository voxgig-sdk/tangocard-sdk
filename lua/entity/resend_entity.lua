...
local ResendEntity = {}
ResendEntity.__index = ResendEntity


function ResendEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResendEntity)
  self._name = "resend"
  self._client = client
...
function ResendEntity:get_name()
  return self._name
end


function ResendEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResendEntity.new(self._client, opts)
end
...
function ResendEntity:mark_deleted()
  self._deleted = true
end


function ResendEntity:deleted()
  return true == self._deleted
end


function ResendEntity:data_set(args)
  if args ~= nil then
...
function ResendEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResendEntity:match_set(args)
  if args ~= nil then
...
function ResendEntity:match_get()
...
function ResendEntity:stream(action, args, callopts)
...
---@param reqdata ResendCreateData
---@param ctrl? table
---@return Resend
---@return string? err
function ResendEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ResendEntity:_run_op(ctx, post_done)
  local utility = self._utility
