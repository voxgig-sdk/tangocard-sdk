-- Tangocard SDK AsyncOrderLineItemsView entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AsyncOrderLineItemsViewEntity = {}
AsyncOrderLineItemsViewEntity.__index = AsyncOrderLineItemsViewEntity


function AsyncOrderLineItemsViewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AsyncOrderLineItemsViewEntity)
  self._name = "async_order_line_items_view"
  self._client = client
...
function AsyncOrderLineItemsViewEntity:get_name()
  return self._name
end


function AsyncOrderLineItemsViewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AsyncOrderLineItemsViewEntity.new(self._client, opts)
end
...
function AsyncOrderLineItemsViewEntity:mark_deleted()
  self._deleted = true
end


function AsyncOrderLineItemsViewEntity:deleted()
  return true == self._deleted
end


function AsyncOrderLineItemsViewEntity:data_set(args)
  if args ~= nil then
...
function AsyncOrderLineItemsViewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AsyncOrderLineItemsViewEntity:match_set(args)
  if args ~= nil then
...
function AsyncOrderLineItemsViewEntity:match_get()
...
function AsyncOrderLineItemsViewEntity:stream(action, args, callopts)
...
---@param reqmatch AsyncOrderLineItemsViewListMatch
---@param ctrl? table
---@return AsyncOrderLineItemsView[]
---@return string? err
function AsyncOrderLineItemsViewEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function AsyncOrderLineItemsViewEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AsyncOrderLineItemsViewEntity
