-- Tangocard SDK AsyncUpdateLineItemView entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AsyncUpdateLineItemViewEntity = {}
AsyncUpdateLineItemViewEntity.__index = AsyncUpdateLineItemViewEntity


function AsyncUpdateLineItemViewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AsyncUpdateLineItemViewEntity)
  self._name = "async_update_line_item_view"
  self._client = client
...
function AsyncUpdateLineItemViewEntity:get_name()
  return self._name
end


function AsyncUpdateLineItemViewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AsyncUpdateLineItemViewEntity.new(self._client, opts)
end
...
function AsyncUpdateLineItemViewEntity:mark_deleted()
  self._deleted = true
end


function AsyncUpdateLineItemViewEntity:deleted()
  return true == self._deleted
end


function AsyncUpdateLineItemViewEntity:data_set(args)
  if args ~= nil then
...
function AsyncUpdateLineItemViewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AsyncUpdateLineItemViewEntity:match_set(args)
  if args ~= nil then
...
function AsyncUpdateLineItemViewEntity:match_get()
...
function AsyncUpdateLineItemViewEntity:stream(action, args, callopts)
...
---@param reqdata AsyncUpdateLineItemViewUpdateData
---@param ctrl? table
---@return AsyncUpdateLineItemView
---@return string? err
function AsyncUpdateLineItemViewEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function AsyncUpdateLineItemViewEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AsyncUpdateLineItemViewEntity
