...
local N8LineItemEntity = {}
N8LineItemEntity.__index = N8LineItemEntity


function N8LineItemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, N8LineItemEntity)
  self._name = "n8_line_item"
  self._client = client
...
function N8LineItemEntity:get_name()
  return self._name
end


function N8LineItemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return N8LineItemEntity.new(self._client, opts)
end
...
function N8LineItemEntity:mark_deleted()
  self._deleted = true
end


function N8LineItemEntity:deleted()
  return true == self._deleted
end


function N8LineItemEntity:data_set(args)
  if args ~= nil then
...
function N8LineItemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function N8LineItemEntity:match_set(args)
  if args ~= nil then
...
function N8LineItemEntity:match_get()
...
function N8LineItemEntity:stream(action, args, callopts)
...
---@param reqdata N8LineItemUpdateData
---@param ctrl? table
---@return N8LineItem
---@return string? err
function N8LineItemEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function N8LineItemEntity:_run_op(ctx, post_done)
  local utility = self._utility
