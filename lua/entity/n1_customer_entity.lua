...
local N1CustomerEntity = {}
N1CustomerEntity.__index = N1CustomerEntity


function N1CustomerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, N1CustomerEntity)
  self._name = "n1_customer"
  self._client = client
...
function N1CustomerEntity:get_name()
  return self._name
end


function N1CustomerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return N1CustomerEntity.new(self._client, opts)
end
...
function N1CustomerEntity:mark_deleted()
  self._deleted = true
end


function N1CustomerEntity:deleted()
  return true == self._deleted
end


function N1CustomerEntity:data_set(args)
  if args ~= nil then
...
function N1CustomerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function N1CustomerEntity:match_set(args)
  if args ~= nil then
...
function N1CustomerEntity:match_get()
...
function N1CustomerEntity:stream(action, args, callopts)
...
---@param reqmatch N1CustomerLoadMatch
---@param ctrl? table
---@return N1Customer
---@return string? err
function N1CustomerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function N1CustomerEntity:_run_op(ctx, post_done)
  local utility = self._utility
