...
local AsyncOrderEntity = {}
AsyncOrderEntity.__index = AsyncOrderEntity


function AsyncOrderEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AsyncOrderEntity)
  self._name = "async_order"
  self._client = client
...
function AsyncOrderEntity:get_name()
  return self._name
end


function AsyncOrderEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AsyncOrderEntity.new(self._client, opts)
end
...
function AsyncOrderEntity:mark_deleted()
  self._deleted = true
end


function AsyncOrderEntity:deleted()
  return true == self._deleted
end


function AsyncOrderEntity:data_set(args)
  if args ~= nil then
...
function AsyncOrderEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AsyncOrderEntity:match_set(args)
  if args ~= nil then
...
function AsyncOrderEntity:match_get()
...
function AsyncOrderEntity:stream(action, args, callopts)
...
---@param reqmatch AsyncOrderListMatch
---@param ctrl? table
---@return AsyncOrder[]
---@return string? err
function AsyncOrderEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AsyncOrderCreateData
---@param ctrl? table
---@return AsyncOrder
---@return string? err
function AsyncOrderEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AsyncOrderEntity:_run_op(ctx, post_done)
  local utility = self._utility
