...
describe("AsyncOrderLineItemsViewDirect", function()
  it("should direct-list-async_order_line_items_view", function()
    local setup = async_order_line_items_view_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-async_order_line_items_view", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"account01", "customer01", "external_ref01"}) do
