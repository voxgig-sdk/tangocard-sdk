...
describe("LineItemDirect", function()
  it("should direct-list-line_item", function()
    local setup = line_item_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-line_item", setup.live and "live" or "unit")
  ...
  it("should direct-load-line_item", function()
    local setup = line_item_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-line_item", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function line_item_direct_setup(mockres)
