-- LowBalanceAlertListView direct test
...
describe("LowBalanceAlertListViewDirect", function()
  it("should direct-list-low_balance_alert_list_view", function()
    local setup = low_balance_alert_list_view_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-low_balance_alert_list_view", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"account_identifier01", "customer_identifier01"}) do
    ...
    if setup.live then
      ...
      params["customer_identifier"] = "direct01"
    end
...
function low_balance_alert_list_view_direct_setup(mockres)
