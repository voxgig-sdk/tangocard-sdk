// Command Tangocard-mcp is an MCP server exposing the Tangocard
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Tangocard_list  — list records of an entity
//	Tangocard_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: TANGOCARD_APIKEY carries the API key and
	// TANGOCARD_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TANGOCARD_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TANGOCARD_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTangocardSDK(opts)
