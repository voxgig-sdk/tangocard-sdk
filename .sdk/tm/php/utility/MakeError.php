...
class TangocardMakeError
{
    public static function call(?TangocardContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TangocardContext([], null);
        }
        $op = $ctx->op ?? new TangocardOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TangocardResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TangocardError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TangocardError('', $msg, $ctx);
        ...
        if ($err instanceof TangocardError) {
