...
{
    private static function utility(): TangocardUtility
    {
        return new TangocardUtility();
    }
    ...
    private static function ctx(array $over = []): TangocardContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TangocardContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TangocardOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TangocardError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TangocardOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TangocardMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TangocardOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TangocardMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TangocardOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TangocardMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TangocardSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TangocardMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TangocardResponse([]),
            'result' => new TangocardResult([]),
        ]);
        [, $err] = TangocardMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TangocardSpec([]),
            'response' => null,
            'result' => new TangocardResult([]),
        ]);
        [, $err] = TangocardMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TangocardSpec(['step' => 's']),
            'response' => new TangocardResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TangocardResult([]),
        ]);
        [, $err] = TangocardMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TangocardSpec(['step' => 's']),
            'response' => new TangocardResponse(self::resp(200, ['v' => 1])),
            'result' => new TangocardResult([]),
        ]);
        [, $err] = TangocardMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TangocardSpec(['step' => 's']),
            'response' => new TangocardResponse(self::resp(200, ['v' => 2])),
            'result' => new TangocardResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TangocardMakeResponse::call($ctx);
    ...
    {
        $preset = new TangocardResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TangocardSpec([]),
            'response' => new TangocardResponse([]),
            'result' => new TangocardResult([]),
        ...
        [$got, $err] = TangocardMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TangocardResult([])]);
        [, $err] = TangocardMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TangocardSpec([]), 'result' => null]);
        [, $err] = TangocardMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TangocardSpec(['step' => 's']),
            'result' => new TangocardResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TangocardOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TangocardMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TangocardSpec(['step' => 's']),
            'result' => new TangocardResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TangocardOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TangocardMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TangocardResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TangocardSpec([]),
            'result' => new TangocardResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TangocardMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TangocardMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TangocardError('boom', 'boom');
        $utility->fetcher = function (TangocardContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TangocardSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TangocardMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TangocardContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TangocardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TangocardResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TangocardContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TangocardSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TangocardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TangocardResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TangocardSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TangocardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TangocardMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TangocardMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TangocardResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TangocardDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TangocardResult(['ok' => false])]);
        $this->expectException(TangocardError::class);
        TangocardDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TangocardResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TangocardMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TangocardMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TangocardBaseFeature();
        $b = new TangocardBaseFeature();
        TangocardFeatureAdd::call($ctx, $a);
        TangocardFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TangocardBaseFeature
    {
        $f = new TangocardBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TangocardFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TangocardContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TangocardSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TangocardPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TangocardResponse(['headers' => null]),
            'result' => new TangocardResult([]),
        ]);
        TangocardResultHeaders::call($ctx);
