...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Catalog()` — with named operations (`list`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
