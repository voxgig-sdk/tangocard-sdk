...
class FtCtrl extends TangocardControl
{
...
class FtClient
{
    ...
    private TangocardUtility $utility;

    public function __construct(TangocardUtility $utility, array $options)
    {
    ...
    public function get_utility(): TangocardUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TangocardContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TangocardContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TangocardContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TangocardConfig::make_feature($name);
    ...
    private static function buildUrl(TangocardSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TangocardError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TangocardResponse($fetched);
        }
    ...
    private function fail(TangocardContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TangocardContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
